...
Matrix6d Rigid::adjoint(const Matrix4d &E)
{
	...
	Matrix3d R = bracket3(E.block<3, 1>(0, 3)) * E.block<3,3>(0,0);
	//Vector3d p = E.block<3,1>(0,3);
	//Ad.block(0,0,3,3) = R;
...
Matrix6d Rigid::addot(const Eigen::Matrix4d &E, const Vector6d &phi)
{
	ChronoTimer time("rigid", 5);
	time.tic(0);
	time.tic(1);
	Matrix6d Ad = Matrix6d::Zero();
	...
	time.toc(1);
	time.tic(2);
	Eigen::Matrix3d Rwbrac = R * bracket3(phi.segment<3>(0));
	time.toc(2);
	time.tic(3);
	Eigen::Matrix3d Rvbrac_pbracRwbrac = R * bracket3(phi.segment<3>(3)) + bracket3(E.block<3, 1>(0, 3))*Rwbrac;
	time.toc(3);
	time.tic(4);
	for (int r = 0; r < 3; ++r)
	{
		...
		{
			Ad(r, c) = Rwbrac(r, c);
			Ad(r + 3, c) = Rvbrac_pbracRwbrac(r, c);
			Ad(r + 3, c + 3) = Rwbrac(r, c);
		}
	}
	time.toc(4);
	//Matrix6d Ad = Matrix6d::Zero();
	//Eigen::Matrix3d R = E.block<3, 3>(0, 0);
	//Eigen::Vector3d p = E.block<3, 1>(0, 3);
	//Eigen::Vector3d w = phi.segment<3>(0);
	//Eigen::Vector3d v = phi.segment<3>(3);
	//Eigen::Matrix3d Rwbrac = R * bracket3(w);
	//Eigen::Matrix3d Rvbrac_pbracRwbrac = R * bracket3(v) + bracket3(p) * Rwbrac;
	//for (int r = 0; r < 3; ++r)
	//{
	//	for (int c = 0; c < 3; ++c)
	//	{
...
Vector6d Rigid::log(const Eigen::Matrix4d & A)
{
	...
	if (abs(theta) < 1e-8)  
	{
		...
		phi.segment<3>(3) = A.block<3, 1>(0, 3);
	}
	else
	{
		...
		phi.segment<3>(3) = V.householderQr().solve(A.block<3, 1>(0, 3));
	}
	return phi;
}
