classdef (Abstract) Body < handle
	...
	properties
		...
		wext_i   % External wrench in body space (not used by redmax)
		fc_i     % Contact force from joint
		damping  % Viscous damping
		Kmdiag   % Block diagonal stiffness
		Dmdiag   % Block diagonal damping
		Ddiag    % Diagonal damping for preconditioner
	...
	methods
		...
		function [M,f] = computeMassGrav(this,grav,M,f)
			...
			f(rows) = fcor + fgrav;
			% External wrench: used only by recurse (not redmax) to
			% accumulate the wrenches to be applied to the joint in
			% rhdPass2(). For redmax, the array of wrenches is used to add
			% wrenches to the bodies directly (Spring objects).
			this.wext_i = zeros(6,1);
			this.Kmdiag = zeros(6);
			this.Dmdiag = zeros(6);
			this.Ddiag = zeros(6);
		...
		function [f,D] = computeForceDamping(this,f,D)
			...
			if this.damping > 0
				...
				% These will be used by the recursive algorithm (not
				% redmax)
				this.wext_i = this.wext_i + fi;
				% For preconditioner
				this.Ddiag = this.Ddiag + Di;
			end
			if ~isempty(this.next)
		...
		%% DEBUG
		function K = getKmdiag(this,K)
			if nargin == 1
				nm = redmax.Scene.countM();
				K = zeros(nm);
			end
			K(this.idxM,this.idxM) = K(this.idxM,this.idxM) + this.Kmdiag;
			if ~isempty(this.next)
				K = this.next.getKmdiag(K);
			end
		end
