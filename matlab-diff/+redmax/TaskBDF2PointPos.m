classdef TaskBDF2PointPos < redmax.TaskBDF2
	%TaskBDF2PointPos Sets a point on a rigid body to be at a target point.
	...
	methods
		%%
		function this = TaskBDF2PointPos(scene)
			nparams = 0;
			for i = 1 : length(scene.joints)
				nparams = nparams + scene.joints{i}.ndof;
			end
			this = this@redmax.TaskBDF2(scene,nparams);
		end
		...
		function calcStep(this)
			...
			% With BDF2, the force term of g is:
			%   g = ... - (4/9)*h^2*(fr + J'*fm + fqvv)
			% The parameters are the torques scaled by pscale, which go
			% into fr, so:
			%   dg/dp = -(4/9)*h^2*pscale*I
			h = this.scene.h;
			kk = (k-1)*nr + (1:nr); % indices for time step k
			this.dgdp(kk,:) = -(4/9)*h^2*this.pscale*speye(nr);
		end
